...
class ZarinpalPaymentRequestView(APIView):
    ...
    def post(self, request):
        ...
        if not order_id:
            return Response(
                {"error": "order_id ارسال نشده"}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
            order = OrderModels.objects.get(id=order_id, user=request.user)
        except OrderModels.DoesNotExist:
            return Response(
                {"detail": "سفارشی با این شناسه یافت نشد."},
        ...
        return Response(
            {"error": "خطا در ایجاد درخواست پرداخت"}, status=status.HTTP_400_BAD_REQUEST
        )
...
class ZarinpalPaymentVerifyView(APIView):
    ...
    def get(self, request):
        ...
        if not authority:
            return Response(
                {"error": "پارامتر Authority ارسال نشده"},
        ...
        if result.get("data") and result["data"].get("code") == 100:
            ...
            payment.save()
            return Response(
                {"message": "پرداخت ناموفق بود."}, status=status.HTTP_400_BAD_REQUEST
            )
