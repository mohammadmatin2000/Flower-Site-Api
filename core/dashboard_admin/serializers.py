...
class AdminSecuritySerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        if attrs['new_password'] != attrs['confirm_password']:
            raise serializers.ValidationError({"confirm_password": "رمز عبور جدید با تاییدیه آن مطابقت ندارد."})
        return attrs
...
class AdminProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PlantProduct
        fields = ['id', 'name', 'slug', 'price', 'status', 'created_date', 'updated_date', 'detail_url', 'list_url']
    def get_detail_url(self, obj):
        request = self.context.get('request')
